...
cat <<EOF | kind create cluster --name=cilium-kpr-ext-to-clusterip --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-kpr-ext-to-clusterip
...
# 1.0: External Access To ClusterIP Services[https://docs.cilium.io/en/stable/network/kubernetes/kubeproxy-free/#external-access-to-clusterip-services]
# 1.1: Options(--set bpf.lbExternalClusterIP=true)

helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-kpr-ext-to-clusterip --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set bpf.lbExternalClusterIP=true
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-kpr-ext-to-clusterip);do docker exec $container ls -al /proc/self/ns/cgroup;done
