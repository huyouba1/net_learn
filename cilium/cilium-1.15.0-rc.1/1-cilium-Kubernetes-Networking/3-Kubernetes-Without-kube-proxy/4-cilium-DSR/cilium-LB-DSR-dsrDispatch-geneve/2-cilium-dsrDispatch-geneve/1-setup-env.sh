...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-lb-dsr-dispatch
...
# 1.1: https://docs.cilium.io/en/v1.15/network/kubernetes/kubeproxy-free/#direct-server-return-dsr-with-geneve
# 1.2: https://github.com/cilium/cilium/issues/30845
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-lb-dsr-dispatch --set kubeProxyReplacement=true --set routingMode=tunnel --set tunnelProtocol=geneve --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set bpf.masquerade=true --set loadBalancer.mode=dsr --set loadBalancer.dsrDispatch=geneve
