...
cat <<EOF | kind create cluster --name=cilium-session-affinity --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-session-affinity
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-session-affinity);do docker exec $container ls -al /proc/self/ns/cgroup;done
