...
cat <<EOF | kind create cluster --name=cilium-big-tcp-ipv4-ipv6 --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-big-tcp-ipv4-ipv6
...
# 0.0: eBPF Host-Routing//Kernel >= 5.19|6.3//eBPF-based kube-proxy replacement//eBPF-based masquerading//Tunneling and encryption disabled//Supported NICs:mlx4,mlx5,ice
# 1.0: IPv4 BIG TCP Options(--set ipv4.enabled=true --set enableIPv4BIGTCP=true --set ipv4NativeRoutingCIDR="10.0.0.0/8")
# 1.1: IPv6 BIG TCP Options(--set ipv6.enabled=true --set enableIPv6BIGTCP=true --set enableIPv6Masquerade=false)

helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-big-tcp-ipv4-ipv6 --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set bpf.masquerade=true --set ipv4.enabled=true --set enableIPv4BIGTCP=true --set ipv6.enabled=true --set enableIPv6BIGTCP=true --set enableIPv6Masquerade=false
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-big-tcp-ipv4-ipv6);do docker exec $container ls -al /proc/self/ns/cgroup;done
