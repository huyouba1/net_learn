...
cat <<EOF | kind create cluster --name=cilium-kpr-ebpf-maglev --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-kpr-ebpf-maglev
...
# 1.0: Maglev Consistent Hashing(SEED=$(head -c12 /dev/urandom|base64 -w0) --set loadBalancer.algorithm=maglev --set maglev.tableSize=65521 --set maglev.hashSeed=$SEED)
# https://docs.cilium.io/en/v1.15/network/kubernetes/kubeproxy-free/#maglev-consistent-hashing
# cilium eBPF kube-proxy replacement 使用了The Maglev paper 论文介绍的变种hash算法来实现在负载均衡后端endpoint一致性hash。这种算法提高了更好的负载平衡和故障恢复能力。集群中的某个节点在不用考虑同步其它节点的状态时就可以对某个5元组做出一致的后端选择。而在某个后端被删除后，后端查找表重新排列时对不相关的后端实例影响最小（重新分配后的一致性hash差异率约为1%）.请注意，Maglev hash仅适用于外部（N-S）流量。对于集群内服务连接（E-W），套接字直接分配给服务后端，例如在TCP连接时，没有任何中间跳，因此不受Maglev的影响。Cilium的XDP加速也支持Maglev hash.
SEED=$(head -c12 /dev/urandom|base64 -w0)
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-kpr-ebpf-maglev --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set bpf.masquerade=true --set loadBalancer.algorithm=maglev --set maglev.tableSize=65521 --set maglev.hashSeed=$SEED
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-kpr-ebpf-maglev);do docker exec $container ls -al /proc/self/ns/cgroup;done
