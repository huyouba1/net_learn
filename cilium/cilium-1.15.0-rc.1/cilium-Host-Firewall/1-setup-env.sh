...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-host-firewall
...
# 1.0: Host firewall Options(--set hostFirewall.enabled=true --set devices='{ethX,ethY}'[--set devices=eth+{if with k8s-node network interface, permit 6443 firstly}])
# 1.1: https://docs.cilium.io/en/v1.15/security/host-firewall/
# 1.2: 
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-host-firewall --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set bpf.masquerade=true --set hostFirewall.enabled=true --set devices=eth1
