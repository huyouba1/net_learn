...
cat <<EOF | kind create cluster --name=cilium-lb-ipam --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-lb-ipam
...
# 1.0: LB IPAM is a feature that allows Cilium to assign IP addresses to Services of type LoadBalancer. This functionality is usually left up to a cloud provider, however, when deploying in a private cloud environment, these facilities are not always available.
# 1.1: LB IPAM works in conjunction with features such as Cilium BGP Control Plane (Beta) and L2 Announcements / L2 Aware LB (Beta). Where LB IPAM is responsible for allocation and assigning of IPs to Service objects and other features are responsible for load balancing and/or advertisement of these IPs.
# 1.2: Use Cilium BGP Control Plane (Beta) to advertise the IP addresses assigned by LB IPAM over BGP and L2 Announcements / L2 Aware LB (Beta) to advertise them locally
# 1.3: LB IPAM is always enabled but dormant. The controller is awoken when the first IP Pool is added to the cluster.[https://docs.cilium.io/en/stable/network/lb-ipam/#lb-ipam]
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-lb-ipam --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8"
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-lb-ipam);do docker exec $container ls -al /proc/self/ns/cgroup;done
