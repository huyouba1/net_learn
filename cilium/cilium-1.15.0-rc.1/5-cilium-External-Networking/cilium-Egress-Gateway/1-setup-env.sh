...
cat <<EOF | kind create cluster --name=cilium-egress-gateway --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kubectl taint node cilium-egress-gateway-worker2 egress-gw:NoSchedule
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-egress-gateway
...
# 1.0: Egress gateway Options(--set egressGateway.enabled=true --set l7Proxy=false --set devices=eth+ --set bpf.masquerade=true)
# 1.1: Egress Gateway requires a 5.2 or more recent kernel. https://docs.cilium.io/en/v1.15/network/egress-gateway/
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-egress-gateway --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set bpf.masquerade=true --set egressGateway.enabled=true --set l7Proxy=false --set devices=eth+
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-egress-gateway);do docker exec $container ls -al /proc/self/ns/cgroup;done
