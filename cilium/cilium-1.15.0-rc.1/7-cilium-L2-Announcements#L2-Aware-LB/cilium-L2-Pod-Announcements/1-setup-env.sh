...
cat <<EOF | kind create cluster --name=cilium-l2-announcements-l2lb-pod-announcements --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-l2-announcements-l2lb-pod-announcements);do docker exec $container ls -al /proc/self/ns/cgroup;done
