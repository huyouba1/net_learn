...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-gwapi-https
...
# 1.0: TLS with minica[Self-signed Certificate]
...
# 1.1: Or with CertManager(https://docs.cilium.io/en/stable/network/servicemesh/gateway-api/https/#create-tls-certificate-and-private-key)


kubectl create secret tls demo-cert --key=./minica/_.cilium.rocks/key.pem --cert=./minica/_.cilium.rocks/cert.pem

helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-gwapi-https --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set gatewayAPI.enabled=true --set l7Proxy=true
