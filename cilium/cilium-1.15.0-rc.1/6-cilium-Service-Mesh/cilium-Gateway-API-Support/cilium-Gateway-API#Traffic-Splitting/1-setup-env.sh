...
cat <<EOF | kind create cluster --name=cilium-gwapi-http-traffic-splitting --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version --name cilium-gwapi-http-traffic-splitting
...
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.15.0-rc.1 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-gwapi-http-traffic-splitting --set kubeProxyReplacement=true --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set gatewayAPI.enabled=true --set l7Proxy=true
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-gwapi-http-traffic-splitting);do docker exec $container ls -al /proc/self/ns/cgroup;done
