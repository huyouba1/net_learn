...
# 8. Appply DNS Policies Using Patterns
cat ./cilium-DNS-Policies-Using-Patterns.yaml
# 8.1: he above policy controlled DNS access based on exact match of the DNS domain name. Often, it is required to allow access to a subset of domains. Let’s say, in the above example, mediabot pods need access to any GitHub sub-domain, e.g., the pattern *.github.com. We can achieve this easily by changing the toFQDN rule to use matchPattern instead of matchName.

# 8.2: Test that mediabot has access to multiple GitHub services for which the DNS matches the pattern *.github.com. It is important to note and test that this doesn’t allow access to github.com because the *. in the pattern requires one subdomain to be present in the DNS name. You can simply add more matchName and matchPattern clauses to extend the access. (See DNS based policies to learn more about specifying DNS rules using patterns and names.)

kubectl apply -f ./cilium-DNS-Policies-Using-Patterns.yaml
