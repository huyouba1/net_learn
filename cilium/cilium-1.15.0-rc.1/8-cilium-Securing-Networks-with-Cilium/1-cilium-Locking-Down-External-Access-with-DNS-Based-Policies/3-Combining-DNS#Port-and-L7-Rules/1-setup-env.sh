...
# 8. Combining DNS, Port and L7 Rules
cat ./cilium-Combining-DNS#Port-and-L7-Rules.yaml
# 8.1: The DNS-based policies can be combined with port (L4) and API (L7) rules to further restrict the access. In our example, we will restrict mediabot pods to access GitHub services only on ports 443. The toPorts section in the policy below achieves the port-based restrictions along with the DNS-based policies.
# 8.2: Testing, the access to https://support.github.com on port 443 will succeed but the access to http://support.github.com on port 80 will be denied.

kubectl apply -f ./cilium-Combining-DNS#Port-and-L7-Rules.yaml
