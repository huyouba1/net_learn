...
# 8. Appply DNS Egress Policy
cat ./cilium-DNS-Egress-policy.yaml
# 8.1: The first egress section uses toFQDNs: matchName specification to allow egress to api.github.com. The destination DNS should match exactly the name specified in the rule. The endpointSelector allows only pods with labels class: mediabot, org:empire to have the egress access.
# 8.2: The second egress section (toEndpoints) allows mediabot pods to access kube-dns service. Note that rules: dns instructs Cilium to inspect and allow DNS lookups matching specified patterns. In this case, inspect and allow all DNS queries.
# 8.3: Note that with this policy the mediabot doesn’t have access to any internal cluster service other than kube-dns. Refer to Overview of Network Policy to learn more about policies for controlling access to internal cluster services.
# 8.4: Testing the policy, we see that mediabot has access to gitee.com but doesn’t have access to any other external service, e.g., support.gitee.com.

kubectl apply -f ./cilium-DNS-Egress-policy.yaml
