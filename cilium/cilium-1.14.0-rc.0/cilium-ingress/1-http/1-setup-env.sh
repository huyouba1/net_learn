...
cat <<EOF | kind create cluster --name=cilium-ingress-http --image=kindest/node:v1.27.3 --config=-
kind: Cluster
...
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.14.0-rc.0 --namespace kube-system --set debug.enabled=true --set debug.verbose=datapath --set monitorAggregation=none --set ipam.mode=cluster-pool --set cluster.name=cilium-ingress-http --set kubeProxyReplacement=true --set tunnel=disabled --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set ingressController.enabled=true --set ingressController.loadbalancerMode=dedicated --set ingressController.default=true --set l7Proxy=true
...
for container in $(docker ps -a --format "table {{.Names}}" | grep cilium-ingress-http);do docker exec $container ls -al /proc/self/ns/cgroup;done
