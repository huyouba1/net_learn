...
for ((i=0; i<${1:-2}; i++))
do
  multipass launch 22.04 -n vmnp"$i" -c 3 -m 2G -d 30G --cloud-init - <<EOF
  # cloud-config
  runcmd:
...
mapfile -t ip_addresses < <(multipass list | grep -E 'vmnp' | awk '{print $3}')
...
for ((ip_id=0; ip_id<${#ip_addresses[@]}; ip_id++)); do
    ...
    if [ $ip_id -eq 0 ]; then
	k3sup install --ip=$master_ip --user=root --merge --sudo --cluster --k3s-version=v1.27.3+k3s1 --k3s-extra-args "--flannel-backend=none --cluster-cidr=10.244.0.0/16 --disable-network-policy --disable traefik --disable servicelb --node-ip=$master_ip" --local-path $HOME/.kube/config --context=nokprp
    else
