...
mapfile -t ip_addresses < <(multipass list | grep vm2410[0-9] | awk '{print $3}')
...
for ((ip_id=0; ip_id<${#ip_addresses[@]}; ip_id++)); do
    ...
    k3sup install --ip=$master_ip --user=root --merge --sudo --cluster --k3s-version=v1.27.3+k3s1 --k3s-extra-args "--flannel-backend=none --cluster-cidr=10.244.0.0/16 --disable-network-policy --disable traefik --disable servicelb --node-ip=$master_ip" --local-path $HOME/.kube/config --context=2410ctx
done
