...
for ((i=0; i<${1:-1}; i++))
do
  multipass launch 23.04 -n vm2310"$i" -c $cpu -m $mem -d 30G --cloud-init - <<EOF
  # cloud-config
  runcmd:
    ...
    - sudo wget -r -np -nH --cut-dirs=3 --directory-prefix="/root/" http://192.168.2.100/k3s/vmenv/mmenv/ubuntu2310/ && chmod +x "/root/ubuntu2310/pwru.sh" 
    - sudo find /root/ubuntu2310/ -name index.html -exec rm {} \;
EOF
done

mapfile -t ip_addresses < <(multipass list | grep vm2310[0-9] | awk '{print $3}')
...
for ((ip_id=0; ip_id<${#ip_addresses[@]}; ip_id++)); do
    ...
    echo "${ip_addresses[$ip_id]} vm2310$ip_id" >> /etc/hosts
done
